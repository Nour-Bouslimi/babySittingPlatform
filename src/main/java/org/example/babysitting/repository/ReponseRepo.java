...
public interface ReponseRepo extends JpaRepository<Reponse, Long> {
    ...
    List<Reponse> findByDate(Date date);
    ...
    List<Reponse> findByIdMsgAndDate(Long idMsg, Date date); //ajout de la méthode pour récupérer les réponses par l'ID du message et la date
    List<Reponse> findByIdUserAndDate(Long idUser, Date date); //ajout de la méthode pour récupérer les réponses par l'ID de l'utilisateur et la date
